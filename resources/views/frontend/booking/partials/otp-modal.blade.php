...
@if(session()->has('booking_data') || session('otp_pending') || $errors->has('otp'))
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
    <div class="relative w-full max-w-md rounded-zen-md bg-white p-6 shadow-zen-md">
      ...
      <form method="POST" action="{{ route('booking.cancel-otp') }}" class="absolute right-4 top-4">@csrf<button type="submit" class="text-2xl leading-none text-zen-muted transition hover:text-zen-text" aria-label="Close OTP popup">&times;</button></form>
      ...
      @if($otpLockSeconds > 0)
        <p id="otp-lock-message" class="mt-3 rounded bg-yellow-100 p-3 text-sm text-yellow-800">You have entered the wrong OTP too many times. Please wait <span id="otp-countdown" data-seconds="{{ $otpLockSeconds }}">{{ $otpLockSeconds }}</span> seconds before trying again.</p>
      @endif
      <form method="POST" action="{{ route('booking.verify.otp') }}" class="mt-4">@csrf
        <input id="otp-input" name="otp" maxlength="6" placeholder="Enter 6-digit OTP" @disabled($otpLockSeconds > 0) class="h-11 w-full rounded border border-zen-border px-3 text-center text-lg font-semibold tracking-widest text-zen-text outline-none focus:border-zen-primary focus:ring-2 focus:ring-zen-primary/20 disabled:cursor-not-allowed disabled:bg-gray-100">
        @error('otp')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
        <button type="submit" id="verify-otp-btn" @disabled($otpLockSeconds > 0) class="mt-4 h-10 w-full rounded bg-zen-primary text-white transition hover:bg-zen-primary-dark disabled:cursor-not-allowed disabled:bg-gray-400">Verify OTP</button>
      </form>
      <form method="POST" action="{{ route('booking.send-email-otp') }}" class="mt-3">@csrf<button type="submit" id="resend-otp-btn" @disabled($otpLockSeconds > 0) class="h-10 w-full rounded border border-zen-primary text-zen-primary transition hover:bg-zen-accent-soft disabled:cursor-not-allowed disabled:opacity-50">Resend OTP</button></form>
    </div>
  </div>
  @push('scripts')
    <script>
