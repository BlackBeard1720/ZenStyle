  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
    <div class="relative w-full max-w-md rounded-zen-md bg-zen-surface p-6 shadow-zen-md">
      ...
      <form method="POST" action="{{ route('booking.verify.otp') }}" class="mt-4">@csrf
        ...
        <button type="submit" id="verify-otp-btn" @disabled($otpLockSeconds > 0) class="mt-4 h-10 w-full rounded bg-zen-primary text-white transition hover:bg-zen-primary-dark disabled:cursor-not-allowed disabled:bg-zen-border-dark">Verify OTP</button>
      </form>
