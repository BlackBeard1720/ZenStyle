<x-staff.layout title="Services" page-name="ServiceManagement">
  <div x-data="{ pageName: `Services` }">
  ...
  <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800 sm:px-6 sm:py-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Services</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage salon services used for appointment booking.</p>
        </div>
        @can('manage-services')
          <a href="{{ route('staff.services.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">
    ...
    <div class="p-5 sm:p-6">
      <form method="GET" action="{{ route('staff.services.index') }}" class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-5">
        <input
          type="text"
          name="keyword"
        ...
        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
          ...
          >
            <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">All categories</option>
            @foreach($categories as $category)
              <option value="{{ $category->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @selected((string) request('category_id') === (string) $category->id)>{{ $category->name }}</option>
            @endforeach
          </select>
        ...
        <a href="{{ route('staff.services.index') }}" class="inline-flex h-11 items-center justify-center rounded-lg border border-gray-300 px-4 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800">Reset</a>
      </form>
        ...
        <div class="w-full overflow-x-auto">
          <table class="min-w-full table-fixed">
            ...
            @forelse($services as $service)
              @php
                $badgeClass = $service->status === 'active'
                  ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500'
                  : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300';
              @endphp
              <tr class="align-top">
                <td class="px-4 py-4 sm:px-6">
                  <p class="text-theme-sm text-gray-500 dark:text-gray-400">#{{ $service->id }}</p>
                </td>
                <td class="px-4 py-4 sm:px-6">
                  <div class="min-w-0">
                ...
                <td class="px-4 py-4 sm:px-6">
                  <p class="truncate text-theme-sm text-gray-500 dark:text-gray-400">{{ $service->category?->name ?? '-' }}</p>
                </td>
                <td class="px-4 py-4 sm:px-6">
                  <p class="line-clamp-2 text-theme-sm text-gray-500 dark:text-gray-400">{{ \Illuminate\Support\Str::limit($service->description, 120) ?: '-' }}</p>
                </td>
                <td class="px-4 py-4 sm:px-6">
                ...
                <td class="px-4 py-4 sm:px-6">
                  <div class="flex items-center justify-end gap-2">
                    <a href="{{ route('staff.services.show', $service) }}" title="View service" class="text-gray-500 hover:text-brand-600 dark:text-gray-400">
                    ...
                    @can('manage-services')
                      <a href="{{ route('staff.services.edit', $service) }}" title="Edit service" class="text-gray-500 hover:text-blue-600 dark:text-gray-400">
                      ...
                      <form action="{{ route('staff.services.destroy', $service) }}" method="POST" onsubmit="return confirm('Delete this service? Services with appointments will be marked inactive instead.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" title="Delete service" aria-label="Delete service" class="text-gray-500 hover:text-error-600 dark:text-gray-400">
                          <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @empty
              <tr>
                <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No services found.</td>
              </tr>
            @endforelse
            </tbody>
          </table>
